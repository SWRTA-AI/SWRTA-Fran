...
class Tokenizer:
    ...
    def conv_token_to_id(self, tokens):
        return [self.token_id_map[x] for x in tokens]
    ...
    def conv_id_to_token(self, mons_ids):
        return [self.id_token_map[x] for x in mons_ids]

    def conv_id_to_name(self, mons_ids):
        return [self.id_name_map[x] for x in mons_ids]
    ...
    def conv_name_to_id(self, names):
        return [self.name_id_map[x] for x in names]
